...
selected_invoices = st.session_state.selected_invoices


#----------------------------------------------------
# Import Adyen links with
# extracted pet ID, Customer Id and
# Invoice ID where possible-               df1
#----------------------------------------------------
...
# st.subheader("Xero AR Report for RAmona - df3")
# st.dataframe(df3)
...
# st.subheader("Import Xll Xero PAYG invoices - df2")
# st.dataframe(df2)
# st.write(len(df2))
...
if selection:
    ...
    selected_customer = df3.iloc[selected_rows]["Customer ID"].values[0]
    ar_pet_id = df3.iloc[selected_rows]["Pet ID"].values[0]


    st.header(selected_invoices)
    st.header(selected_customer)
    st.header(ar_pet_id)

    if pd.isna(selected_customer):
...
#----------------------------------------------------
        ...
        if ar_invoice_id:
                ...
                ar_customer_id = df3.loc[df3["Invoice ID"] ==
                                             ar_invoice_id, "Customer ID"].values[0]
                # st.header("Customer Id is found " + ar_customer_id)


                ar_merchant_reference = df3.loc[df3['Invoice ID'] == ar_invoice_id, 'merchantReference'].iloc[0]
                print(ar_merchant_reference)
                # print("Requesting Pet ID in GUI")
                ...
                print("xxx Pet Id: " + ar_pet_id)

                ar_pet_id = str(ar_pet_id)
                pet_data = functions.get_ezyvet_pet_details()
                ...
                selected_invoices = all_invoices[all_invoices['tl_PetID'] == ar_pet_id]
